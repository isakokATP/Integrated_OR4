<template>
  <Header :searchValue="searchQuery" @search="handleSearchUpdate" />
  <div class="px-6 pt-2">
    ...
    <div class="flex justify-end mb-4">
      <button
        @click="router.push({ name: 'sale-orders-page' })"
        class="relative text-2xl p-2 bg-gray-100 rounded hover:bg-gray-200"
      >
        🛍️
        <span
          v-if="globalNewOrdersCount > 0"
          class="absolute -top-1 -right-1 bg-red-600 text-white rounded-full px-2 text-xs"
        >
...
import {
  newOrdersCount as globalNewOrdersCount,
  setNewOrdersCount,
...
const updateBadge = async () => {
  ...
  try {
    ...
    setNewOrdersCount(
      countNewOrders(result.newOrders.concat(result.allOrders))
    );
  } catch (err) {
